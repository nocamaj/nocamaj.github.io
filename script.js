...
(function() {
    ...
    function initAnimation() {
        try {
            ...
            if (isHomepage && !offCtx) throw new Error('Failed to get 2D context for offscreen canvas');
            ...
            const config = { // config object...
                ...
                textPixelOnColor: 'rgba(250, 255, 255, 1)',
            ...
            let currentPhraseIndex = Math.floor(Math.random() * phrases.length); // Start on a random phrase
            ...
            class GridCell { // GridCell class...
                ...
                update(noiseValue) {
                    ...
                    } else { this.isBorderTextRect = false; this.isWithinTextDisplayInterior = false; }
                    ...
                    if (this.isBorderTextRect && isHomepage) { 
                        ...
                        const isTopB=this.row===borderRectCells.startRow;const isBottomB=this.row===borderRectCells.endRow-1; const isLeftB=this.col===borderRectCells.startCol;const isRightB=this.col===borderRectCells.endCol-1;
                        ...
                        if (noiseValue > config.activationThreshold) { finalTargetAlpha = Math.min(1, (noiseValue - config.activationThreshold) / (1 - config.activationThreshold) * 1.2); }
                        this.colorTemplate = (noiseValue > config.highlightThreshold && finalTargetAlpha > 0.5) ? config.highlightColor : config.baseColor;
                        if (finalTargetAlpha > 0.01 && (this.currentAlpha < 0.1 || Math.random() < config.charChangeProbability)) { this.char = getRandomChar(); } 
                        else if (finalTargetAlpha < 0.01) { this.char = ''; }
                        ...
                        if(distToCursor < config.cursorInteractionRadius){ const proximityFactor=Math.max(0,1-(distToCursor/config.cursorInteractionRadius)); const vanishAmount=Math.pow(proximityFactor,config.cursorVanishFalloff)*config.cursorVanishStrength; cursorVanishMultiplier = (1 - vanishAmount); }
            ...
            function setupAndRun() {
                ...
                // Start animation, and once first frame is drawn, fade in container
                let isFirstFrame = true;
                function animate() {
                    animationFrameId = requestAnimationFrame(animate);
                    const now = performance.now(); const elapsed = now - lastFrameDrawnTime;

                    if (elapsed < frameInterval) return; // FPS cap
                    ...
                    // On the very first frame, add 'loaded' class to fade in the container
                    if (isFirstFrame) {
    ...
    window.addEventListener('load', () => { 
        ...
        if(bgC&&!bgC.querySelector('canvas')){ 
            console.log("Canvas not found on window.load, attempting initAnimation again.");
            initAnimation();
        }
    });
})();